        ...
        public async Task votetwice()
        {
            if (Context.Guild.Id == 580555457983152149)
            {
                ...
                else
                {
                    ...
                    {
                        await Context.Guild.GetTextChannel(580563096544739331).SendMessageAsync("" + Context.Guild.Roles.FirstOrDefault(x => x.Name == "Sống").Mention + ", Hãy bắt đầu bỏ phiều cho người chơi lên giá treo, sử dụng -bophieu (Số Người Chơi Muốn Lên Giá Treo) trong Channel riêng để bỏ phiếu.");
                        GlobalFunctionMaCun.gamestatus++;
                    }
                    else if (GlobalFunctionMaCun.treo != 0 & GlobalFunctionMaCun.gamestatus == 3)
                    {
                        await Context.Guild.GetTextChannel(580563096544739331).SendMessageAsync("" + Context.Guild.Roles.FirstOrDefault(x => x.Name == "Sống").Mention + ", Hãy bắt đầu bỏ phiều sống và chết cho " + Context.Guild.GetUser(GlobalFunctionMaCun.treo).Nickname + ", sử dụng -song [Để Sống] hoặc -chet [Để Giết] trong Channel riêng để bỏ phiếu.");
                        GlobalFunctionMaCun.gamestatus++;
                    }
                    else if (GlobalFunctionMaCun.treo == 0 & GlobalFunctionMaCun.gamestatus == 3)
                    {
                    ...
                    else if (GlobalFunctionMaCun.treo == 1 || GlobalFunctionMaCun.treo == 0)
                    {
        ...
        public async Task calangngu()
        {
            if (Context.Guild.Id == 580555457983152149)
            {
                ...
                else
                {
                    ...
                    {
                        embed.AddField($"Lỗi!", "Vì có người bị treo nên sử dụng -vote để bỏ phiếu sống hoặc chết trước khi đêm (Nếu có sự nhầm lẫn trong việc treo thì sử dụng -treo 0 để bỏ người bị treo).");
                        embed.WithColor(new Discord.Color(255, 0, 0));
                        await Context.Channel.SendMessageAsync("", false, embed.Build());
                    }
                    else if (GlobalFunctionMaCun.treo != 0 & GlobalFunctionMaCun.gamestatus == 4)
                    {
                        if (GlobalFunctionMaCun.votesong >= GlobalFunctionMaCun.votechet)
                        {
                            OverwritePermissions khongchophep = new OverwritePermissions(viewChannel: PermValue.Allow, sendMessages: PermValue.Deny);
                            await Context.Client.GetGuild(580555457983152149).GetTextChannel(580563096544739331).AddPermissionOverwriteAsync(Context.Guild.Roles.FirstOrDefault(x => x.Name == "Sống"), khongchophep.Modify());
                            await Context.Guild.GetTextChannel(580563096544739331).SendMessageAsync("Vì số phiếu Sống nhiều hơn số phiếu Chết hoặc cả 2 bằng nhau nên car làng đi ngủ.");
                            GlobalFunctionMaCun.channel1 = 1;
                            GlobalFunctionMaCun.channel2 = 1;
                            GlobalFunctionMaCun.channel3 = 1;
                            GlobalFunctionMaCun.channel4 = 1;
                    ...
                    if (GlobalFunctionMaCun.can != 0)
                    {
                        ...
                        else
                        {
                            ...
                            else if (GlobalFunctionMaCun.plr5 == GlobalFunctionMaCun.can)
                            {
                                ...
                                {
                                    await Context.Client.GetGuild(580555457983152149).GetTextChannel(580574598677135390).SendMessageAsync("Đêm qua bạn đã cắn hoặc bị đâm, bạn đã mất đi 1 mạng, nếu đợt tiếp theo bị tấn công bạn sẽ chết.");
                                    await Context.Client.GetGuild(580555457983152149).GetTextChannel(580574812662136836).SendMessageAsync("Mục Tiêu của bạn không thể bị tiêu diệt.");
